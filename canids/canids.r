...
setwd("~/Git/fossilDating/canids")
fossilTable_c <- read.table("canidFossilTable.txt", sep="\t", header=T)
regexp = "\\-[0-9]+\\.log"
seq <- read.table("~/Git/fossilDating/canids/canids-sorted-sequences.txt", header=T, sep="\t")
seq$Sequence <- as.character(seq$Sequence)

result_c <- loadFossilFiles(".", regexp, fossilTable_c, 5001)
height_c <- result_c$heights
df_c <- createSummaryTable(result_c$names, height_c, fossilTable_c, seq, hpdProb=0.99)
...
createPhyloAgeVsGeoAgePDF("1c_phyloAgeVsGeoAge.pdf", df_c, fossilTable_c, ind, orient, min=0, max=38)
createPrecisionVsKnownCharactersFigure("1c_precisionVsKnownCharacters.pdf", df_c, ind, orient);
