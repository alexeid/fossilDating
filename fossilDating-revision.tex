...
The fossil ages ranged from TODO.
...
%TSrev
We applied two phylogenetic models to the penguin data set, \Mstrict{} and \Mrelaxed{}, and we applied \Mstrict{} to the canid dataset. \Mstrict{} assumed a strict morphological clock ($\mu$) and no gamma-distributed rate heterogeneity among sites  \cite{Lewis2001}. Model \Mrelaxed{} \citep{gavryushkina2015bayesian} partitioned the alignment into  partitions (six for the penguins), with the $i$'th partition containing all characters that had $k=i+1$ character states across the sampled taxa. Model \Mrelaxed{} also uses a uncorrelated lognormally-distributed relaxed molecular clock \cite{Drummond2006} with parameters $\mu$ and $S$ for the mean rate and log standard deviation of the rates, and an additional parameter for gamma-distributed rate variation across sites \cite{yang:1994ma}. 
%TODO how was this additional parameter set?
We followed \cite{gavryushkina2015bayesian} in having a broad LogNormal(-5.5, 2) prior on $\mu$ for all analyses and a Gamma($\alpha=0.5396$, $\beta=0.3819$) prior on $S$ for the relaxed clock analyses.
%TSref TODO Alexei please check if the following sentence is correct.
For the parameters of the fossilized birth-death model tree prior, 
we specify the prior below in the section ``Computing the phylogenetic evidence for an age range''.
%we  also followed \cite{gavryushkina2015bayesian}, except with an upper limit on the origin of $T= 80$ for the canids. 
...
\begin{figure}
\includegraphics[width=12cm]{8_penguins_eNprior/8_fossilDatingHist_younger.pdf}
...
\begin{figure}
\includegraphics[width=12cm]{8_penguins_eNprior/8_fossilDatingHist_older.pdf}
...
%TODO: well, we could simply calculate the relative error and compare!?
