...
\response{There are no statements in the revised manuscript that the estimates are precise. We quantify the relative precision in the abstract, and elsewhere we plot the precision and discuss that fact that estimates are relatively more precise in the canid analysis than the penguin analysis.}
...
\response{The estimated log sd of the evolutionary rate is 0.69 in the penguin Mk-8 analysis (see Table 3), which is not much different to estimates of the same parameter for relaxed molecular clocks in, for example, influenza.}
